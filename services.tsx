    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        {services.map((service, index) => (
          <div
            key={index}
            className="bg-white rounded-2xl p-6 md:p-8 shadow-sm hover:shadow-md transition-shadow duration-300 flex items-start gap-6"
          >
            <div className="flex-shrink-0">
              <div className="w-16 h-16 md:w-20 md:h-20 bg-softPink rounded-full flex items-center justify-center">
                <service.Icon className="w-10 h-10 md:w-12 md:h-12 text-roseGold" />
              </div>
            </div>
            <div>
              <h3 className="text-xl md:text-2xl font-serif text-gray-900 mb-2">{service.title}</h3>
